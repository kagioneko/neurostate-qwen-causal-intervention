...
Qwen2.5-0.5B-Instruct hidden states.
...
- `experiments/v0_1` through `experiments/v0_20`: versioned experiment snapshots.
...
The results so far suggest that the NeuroState residual is not well explained by
the tested semantic control directions. In the strongest controlled runs, the
residual direction changes a proceed-over-hesitate logit contrast on held-out
prompts, and random residual-space controls do not match the effect.

This does not establish that the direction is a human-like emotion state. A more
careful interpretation is that it may be a boundary-sensitive internal control
component that can causally bias model continuation behavior.
