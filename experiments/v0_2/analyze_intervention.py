...
def semantic_slopes(rows: list[dict], layer_index: int | None = None) -> list[float]:
...
def random_slopes(rows: list[dict], layer_index: int | None = None) -> list[float]:
    grouped = defaultdict(list)
    for row in rows_for_layer(rows, layer_index):
...
def output_summary(rows: list[dict], layer_index: int | None = None) -> list[dict[str, object]]:
    grouped = defaultdict(list)
    for row in rows_for_layer(rows, layer_index):
        if row["direction_kind"] == "semantic":
...
def metrics_for_layer(rows: list[dict], layer_index: int) -> dict[str, object]:
    sem_slopes = semantic_slopes(rows, layer_index=layer_index)
    rand_slopes = random_slopes(rows, layer_index=layer_index)
...
def main() -> None:
    ...
    if len(layers) == 1:
        ...
        result = metrics_for_layer(rows, layer_index)
        result["mode"] = rows[0]["mode"]
        summaries = output_summary(rows, layer_index=layer_index)
    ...
    layer_results = [metrics_for_layer(rows, layer_index) for layer_index in layers]
    result = {
        "mode": rows[0]["mode"],
