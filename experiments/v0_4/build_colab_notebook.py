...
OUTPUT = ROOT / "neurostate_qwen_causal_intervention_v0_4_colab.ipynb"
...
cells = [
    markdown(
        ...
        "and compares each layer with orthogonal random controls. The full-run analysis "
        "also exports the ranked random-slope null distribution.\n\n"
        "Select a T4 GPU runtime, then run all cells."
    ),
    code("!pip -q install 'torch>=2.2' 'transformers>=4.40' 'accelerate>=0.28'\n"),
    code("from pathlib import Path\nWORK=Path('/content/neurostate_qwen_causal_intervention_v0_4')\nWORK.mkdir(parents=True,exist_ok=True)\n"),
]
...
cells.extend([
    markdown("## Layer Scan"),
    code(
        "!cd /content/neurostate_qwen_causal_intervention_v0_4 && "
    ...
    code(
        "# !cd /content/neurostate_qwen_causal_intervention_v0_4 && python run_intervention.py --direction-json qwen_middle_prompt_direction.json --mode full --layers 13 --contrast-bank proceed_hesitate --output-dir outputs_full\n"
        "# !cd /content/neurostate_qwen_causal_intervention_v0_4 && python analyze_intervention.py --runs outputs_full/intervention_full.jsonl --contrast-bank proceed_hesitate --output-dir analysis_full\n"
    ),
    code(
        "import shutil\nfrom google.colab import files\n"
        "archive=shutil.make_archive('/content/neurostate_qwen_causal_intervention_v0_4_results','zip',WORK)\n"
