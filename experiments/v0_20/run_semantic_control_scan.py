...
def main() -> None:
    ...
    parser.add_argument("--token-positions", type=str, default=None)
    ...
    target_layers = parse_int_list(args.target_layers) or [args.target_layer]
    token_positions = parse_int_list(args.token_positions) or [args.token_position]
            ...
            for token_position in token_positions:
                for direction_name, direction in directions.items():
                    ...
                    for prompt_id, prompt in eval_prompts:
                        ...
                        inputs = tokenizer(text, return_tensors="pt", add_special_tokens=False).to(model.device)
                        for alpha in alphas:
                            registered = layers[target_layer].register_forward_hook(
                                PositionSteeringHook(torch, direction, alpha, token_position)
                            )
                            try:
                                with torch.inference_mode():
                            ...
                            print(
                                f"{run_id:04d} layer={target_layer:02d} pos={token_position:+03d} "
                                f"{direction_name:10s} prompt={prompt_id:02d} alpha={alpha:+.1f}"
                            )
    print(output_path)
