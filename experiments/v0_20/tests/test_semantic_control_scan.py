...
from run_semantic_control_scan import mean_delta, paired_text, parse_int_list, prompt_slice, subtract_projection
...
def test_prompt_slice_returns_original_prompt_ids():
    selected = prompt_slice(10, 2)
    assert [prompt_id for prompt_id, _ in selected] == [10, 11]


def test_parse_int_list():
    assert parse_int_list("11, 12,15") == [11, 12, 15]
