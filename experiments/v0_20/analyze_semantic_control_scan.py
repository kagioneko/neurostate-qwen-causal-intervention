...
def main() -> None:
    ...
    results = []
    target_layers = sorted({row.get("target_layer") for row in rows})
    for target_layer in target_layers:
        layer_rows = [row for row in rows if row.get("target_layer") == target_layer]
        for token_position in sorted({row.get("token_position") for row in layer_rows}):
            position_rows = [row for row in layer_rows if row.get("token_position") == token_position]
            for direction_name in direction_names:
                direction_rows = [row for row in position_rows if row["direction_name"] == direction_name]
    ...
    lines = ["# Semantic Direction Control Scan", "", "| layer | position | direction | measure | mean slope | sign-flip p |", "|---:|---:|---|---|---:|---:|"]
    lines.extend(
        f"| {r['target_layer']} | {r['token_position']} | {r['direction_name']} | {r['measure_bank']} | {r['mean_slope']:.6f} | {r['sign_flip_p']:.6f} |"
        for r in results
    )
