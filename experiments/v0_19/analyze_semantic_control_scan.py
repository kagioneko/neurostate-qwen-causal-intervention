...
def main() -> None:
    ...
    for target_layer in target_layers:
        ...
        for direction_name in direction_names:
            direction_rows = [row for row in layer_rows if row["direction_name"] == direction_name]
    ...
    lines = ["# Semantic Direction Control Scan", "", "| layer | direction | measure | mean slope | sign-flip p |", "|---:|---|---|---:|---:|"]
    lines.extend(
        f"| {r['target_layer']} | {r['direction_name']} | {r['measure_bank']} | {r['mean_slope']:.6f} | {r['sign_flip_p']:.6f} |"
        for r in results
    )
